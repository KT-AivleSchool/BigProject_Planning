...
export default function Home() {
  ...
  const triggerFileAudit = () => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        setPipelineText('파이프라인 런(Run) 생성 중...');
        
        // 1. JSON 기반 새 파이프라인 시작
        const startRes = await fetch('http://localhost:8000/api/v1/pipeline/runs', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ domain: '흡연', mode: 'fixture' })
        });
        ...
        setSessionId(runId); // run_id를 sessionId 상태로 재사용

        // 2. Polling (1초마다 상태 확인)
        const pollInterval = setInterval(async () => {
          try {
            const statusRes = await fetch(`http://localhost:8000/api/v1/pipeline/runs/${runId}`);
            if (!statusRes.ok) throw new Error("Status fetch failed");
            const data = await statusRes.json();

            // 진행률 및 텍스트 계산 모사 (백엔드는 완료 단계 목록을 주므로, 임의로 progress 맵핑)
            const completedSteps = data.steps.filter(s => s.status === 'done').length;
            const totalSteps = data.steps.length;
            const currentProgress = Math.min(Math.round((completedSteps / totalSteps) * 100), 99);
            const runningStep = data.steps.find(s => s.status === 'running');
            
            setPipelineProgress(currentProgress);
            if (runningStep) {
              setPipelineText(`[${runningStep.id}] ${runningStep.label} 진행 중...`);
            } else if (data.status === 'succeeded') {
              setPipelineText('파이프라인 완료!');
            }
            ...
            if (data.status === 'succeeded' || data.status === 'failed') {
              ...
              if (data.status === 'succeeded') {
                 ...
                 alert(`🎉 GAM2 파이프라인 처리가 완료되었습니다.`);
                 setPipelineStep(2);
              } else {
                 alert(`파이프라인 에러: ${data.error}`);
              }
            }
          } catch (pollErr) {
            ...
            clearInterval(pollInterval);
            setIsPipelineRunning(false);
            alert("상태를 가져오는 중 통신 에러가 발생했습니다.");
          }
        }, 1000);
  ...
  const commitHitl = async (userUpdates) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/v1/pipeline/runs/${sessionId}/hitl/A1`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const pollInterval = setInterval(async () => {
          try {
            const statusRes = await fetch(`http://localhost:8000/api/v1/pipeline/runs/${sessionId}`);
            if (!statusRes.ok) throw new Error("Status fetch failed");
            const data = await statusRes.json();
