...
1.  **일괄 통합 드래그앤드롭 및 AI 사전 감리 (Unified Ingestion & Audit)**:
    ...
    *   *성능 (OOM/Cost Guard)*: 토큰 과소비 차단을 위해 각 파일별 상위 15개 행만 `readline()` 스트리밍으로 메모리 제한 선독하며, 1MB 초과 시 자동으로 디스크 임시 파일(`SpooledTemporaryFile`)로 캐싱하여 OOM을 물리적으로 차단합니다.
...
1.  전국 지방자치단체의 **도시계획과, 도로교통과, 환경보건과, 사회복지과** 등 공공 인프라 입지 선정 및 관련 공공 민원/갈등 해결 부서 전반.
2.  **다목적 확장 프레임워크 (OmniSite)**: 업로드하는 데이터셋의 목적(전기차 충전소, 스마트 흡연부스, 어린이 안전구역, 실버 스마트쉼터 등)에 따라 주무 공공부처를 스왑(Swap) 매핑하여 범용 사용.
